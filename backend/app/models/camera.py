...
class Camera(Base):
    ...
    # Recoverable deletion. Cameras own zones, detector configuration and
    # historical events, so hard deletion destroys both coverage and evidence.
    # `is_deleted` controls visibility; attribution remains after restoration.
    ...
    deleted_previous_status: Mapped[str | None] = mapped_column(String(16), nullable=True)
    deleted_previous_ai_enabled: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    restored_at:       Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    restored_by_user_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
    restored_by_email: Mapped[str | None] = mapped_column(String(255), nullable=True)
