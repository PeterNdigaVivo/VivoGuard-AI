...
def daily_visitor_count(
    ...
    *,
    ...
    Priority, when the store HAS an entry/exit line drawn:
      1. visitor_count_in metric (inward line crossings). Counts a
         re-entry as a second visit, but cannot count one person twice.
      2. VisitorTrack — fallback only.
      3. occupancy (per-hour peak).

    With NO entry/exit line the first two swap: an inflated count beats
    no count, and the source label tells the UI which it got.

    Occupancy is the last resort — its per-hour values are MAXes, not
    counts, so its total is "sum of hourly peaks" and intentionally
    fuzzy.
    ...
    # A line crossing is an instantaneous event, so a tracker that restarts
    # every inference slice cannot inflate it. VisitorTrack rows are keyed
    # per (camera, track_id), so one shopper is re-counted on every camera
    # they walk past AND every time the tracker renumbers them — which at a
    # 120s slice is every two minutes. Wherever an entrance line is drawn it
    # is therefore the honest source; unique tracks stay first for stores
    # without one, where an inflated count still beats none. Once Re-ID
    # stamps global_person_id the DISTINCT below collapses the duplicates
    # and either source is sound.
    visitor_zone_ids, via_glass = _pick_visitor_zone_ids(db, cam_ids or [])

    def _from_unique_tracks() -> VisitorCount | None:
        # Sprint 2.2: when the Re-ID pipeline has stamped a cross-camera
        # global_person_id, count DISTINCT identities so the same shopper
        # seen on several cameras counts ONCE. Rows without a global id
        # (Re-ID off, or not yet assigned) fall back to their unique row id
        # via COALESCE, so they each still count once exactly as before.
    ...
    def _from_entry_line() -> VisitorCount | None:
    ...
    def _from_occupancy() -> VisitorCount | None:
        rows = (db.query(_eat_hour_expr(MetricSnapshot.period_start, tz_name).label("h"),
    ...
    order = ((_from_entry_line, _from_unique_tracks) if visitor_zone_ids
             else (_from_unique_tracks, _from_entry_line))
    for source in (*order, _from_occupancy):
        counted = source()
